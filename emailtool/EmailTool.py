...
class EmailTool(object):
    ...
    def format_post(self, post_list):
		...
		for post in post_list:
			...
			# 日期和手机来源
			user_info = "%s %s" %(post.get('mblog', None).get('created_at', None), post.get('mblog', None).get('source', None))
			# 微博链接
			...
			if (pics is not None) and (len(pics) is not 0):
				for pic in pics:
					li = '<li class="articleimgli"><img class="articleimgimg" src="%s" alt="img"></li>' % pic.get('url', None)
					lis = lis + li
			...
			if retweeted_status is not None:
				...
				# 被转发的微博主页
				retweeted_profile_url = retweeted_status.get('user', None).get('profile_url', None)
				# 被转发用户昵称
				retweeted_screen_name = retweeted_status.get('user', None).get('screen_name', None)
				# 被转发微博正文
				retweeted_status_text = retweeted_status.get('text', None)
				# 图片li
				retweeted_status_pics = retweeted_status.get('pics', None)
				retweeted_status_lis = '</div>'
				...
				retweeted_status_html = '<div style="background-color: #efefef;padding: 5px 5px;cursor: pointer;" onclick="gotoweibo2(%s)">' \
										'<div class="articletext"><p class="articletextp"><a href="%s">@%s</a>:%s</p></div>' \
										'<div class="articleimg"><ul class="articleimgul">%s</ul></div>' \
										'<div style="clear: both;"></div></div></div></div>' \
										% (retweeted_scheme, retweeted_profile_url, retweeted_screen_name, retweeted_status_text, retweeted_status_lis)

			card_html = '<div style="background-color: #fff;margin: 10px auto;width: 95%; padding: 10px;max-width: 500px">' \
						'<div>' \
						'<div style="float: left;">' \
						'<img class="headerimg" src="%s" alt="头像">' \
						'</div>' \
						'<div style="float: left; margin-left: 10px">' \
						'<div class="headertitle">' \
						'<a href="%s" style="color: #000!important">' \
						'<span class="headertitlespan">%s</span>' \
						'</a>' \
						'</div>' \
						'<div class="headerinfo"><span class="headerinfospan">%s</span></div>' \
						'</div>' \
						'<div style="clear: both;"></div>' \
						'</div>' \
						'<div style="margin: 5px">' \
						'<!-- article -->' \
						'<div style="padding: 5px 5px;cursor: pointer;" onclick="gotoweibo1(%s)">' \
						'<div class="articletext"><p class="articletextp">%s</p></div>' \
						'<div class="articleimg">' \
						'<ul class="articleimgul">%s</ul>' \
						'</div>' \
						'<div style="clear: both;"></div>' \
						'</div>%s' % (profile_image_url, profile__url, screen_name, user_info, scheme, text, lis, retweeted_status_html)
